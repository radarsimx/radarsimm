...
function write_index(output_dir, class_names)
    ...
    w(fid, '```matlab');
    w(fid, '%% Set license');
    w(fid, 'RadarSim.License.set_license();');
    w(fid, '');
    w(fid, '%% Create transmitter and receiver');
    w(fid, 'tx = RadarSim.Transmitter(f, t, ''channels'', {tx_ch});');
    w(fid, 'rx = RadarSim.Receiver(fs, rf_gain, load_resistor, bb_gain, ''channels'', {rx_ch});');
    w(fid, '');
    w(fid, '%% Create radar system');
